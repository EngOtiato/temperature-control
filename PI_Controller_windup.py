...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 10:46:05 2024
...
Kp = 30.0  # Set the proportional gain
Ki = 0.05  # Set the integral gain
integral = 0  # Initialize the integral term
last_error = 0  # Initialize the last error term
last_time = time.time()

# CSV Constants
time_interval = 5  # Set the time interval for temperature recording in seconds
...
with open("Constant_temp.csv", "w", newline="") as my_csv_file:
...
while True:
    ...
    error = desired_temperature - T3_cal
    ...
    v_2 = min(max((output / 100.0) * 16.0, 0), 16.0)
    ...
    # Posting into CSV
    elapsed_time = int(current_time - start_time)  # Calculate elapsed time
    with open("Constant_temp.csv", "a", newline="") as my_csv_file:
        csv_writer = csv.DictWriter(my_csv_file, fieldnames=fieldnames)
        ...
        print(elapsed_time, temp_input, average_internal_temp, T1_cal, T2_cal, T3_cal, T4_cal, T5_cal, T6_cal, Ambient_temp, hot_plate, Total_Power)
